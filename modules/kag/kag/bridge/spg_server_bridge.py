...
def _default_api_key() -> str:
    return os.getenv("OPENAI_API_KEY") or os.getenv("SILICONFLOW_API_KEY") or ""


def _patch_vectorize_model(vec: Dict[str, Any]) -> Dict[str, Any]:
    """补齐向量模型最小必需字段，兼容不同 OpenSPG 项目配置格式。"""
    if not isinstance(vec, dict):
        vec = {}
    provider = str(vec.get("provider", "")).strip().lower()
    if not vec.get("type"):
        vec["type"] = "ollama" if provider == "ollama" else "openai"
    if not vec.get("model"):
        vec["model"] = os.getenv("OPENAI_EMBEDDING_MODEL") or os.getenv(
            "VECTORIZE_MODEL", "BAAI/bge-m3"
        )
    if not vec.get("base_url"):
        vec["base_url"] = os.getenv("OPENAI_API_BASE", "https://api.siliconflow.cn/v1")
    if not vec.get("api_key"):
        fallback = _default_api_key()
        if fallback:
            vec["api_key"] = fallback
    if not vec.get("timeout"):
        vec["timeout"] = float(os.getenv("OPENAI_TIMEOUT_SECONDS", "120"))
    if not vec.get("name"):
        vec["name"] = vec.get("model") or vec.get("modelId") or "vectorize_model"
    return vec


def _patch_vectorizer_component_config(component_config: Dict[str, Any]) -> Dict[str, Any]:
    cfg = component_config if isinstance(component_config, dict) else {}
    vec = cfg.get("vectorize_model")
    cfg["vectorize_model"] = _patch_vectorize_model(vec if isinstance(vec, dict) else {})
    return cfg
...
class SPGServerBridge:
    ...
    def run_component(self, component_name, component_config, input_data):
        ...
        if component_name == "VectorizerABC":
            component_config = _patch_vectorizer_component_config(component_config)
    ...
    def run_vectorizer_config_check(self, vec_config):
        ...
        config = json.loads(vec_config) if isinstance(vec_config, str) else vec_config
        config = _patch_vectorize_model(config if isinstance(config, dict) else {})
        return VectorizeModelConfigChecker().check(json.dumps(config, ensure_ascii=False))
