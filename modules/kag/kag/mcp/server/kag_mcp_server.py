...
class KagMcpServer(object):
    ...
    @staticmethod
    def _should_use_retrieve_fast_path(query: str) -> bool:
        if not query:
            return False
        retrieve_keywords = (
            "证据",
        ...
        return any(keyword in query for keyword in retrieve_keywords)

    async def _retrieve_data(self, query: str) -> dict:
    ...
    def _add_qa_pipeline_tool(self) -> None:
        async def qa_pipeline(query: str) -> str:
            ...
            """
            if self._should_use_retrieve_fast_path(query):
                data = await self._retrieve_data(query)
        ...
        self._mcp_server.add_tool(qa_pipeline, name="qa-pipeline")
    ...
    def _add_kb_retrieve_tool(self) -> None:
        ...
        self._mcp_server.add_tool(kb_retrieve, name="kb-retrieve")
